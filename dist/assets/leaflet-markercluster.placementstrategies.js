...
L.MarkerCluster.include({
    ...
    _generatePointsCircle: function a(b, c) {
        var d = this._group.options.spiderfyDistanceMultiplier * this._circleFootSeparation * (2 + b)
          , e = d / this._2PI
          , f = this._2PI / b
          , g = [];
        var h = void 0
    ...
    _generatePointsClocksCircles: function a(b, c, d) {
        ...
        var g = this._circleFootSeparation * 1.5
        ...
        for (var p = 1; p <= b; p++) {
            ...
            if (d && l === 1) {
                e[p - 1] = this._regularPolygonVertexPlacement(i - 1, Math.min(f, b), c, n)
            } else {
    ...
    _generatePointsConcentricCircles: function a(b, d) {
        ...
        m[3].noElements = Math.max(b - m[0].noElements - m[1].noElements - m[2].noElements, 0);
        var n = 0;
        var o = m[0];
        for (var p = 1; p <= b; p++) {
            if (m[1].noElements > 0) {
                ...
                if (p > m[0].noElements + m[1].noElements + m[2].noElements && m[3].noElements > 0) {
                    o = m[3];
                    n = m[0].noElements - m[1].noElements - m[2].noElements
                }
            }
            c[p - 1] = this._regularPolygonVertexPlacement(p - n, o.noElements, d, o.distance)
        }
        m.filter(function(a) {
            return a.noElements
        }).map(function(a) {
